 ...
 *
 * @body    { hostId: string, problems: string[], metadata?: object }
 */
export const createRoom = async (req, res) => {
  try {
    const { hostId, problems, metadata } = req.body;
    ...
    console.log(`✅ [Create Room] Room ${roomId} created successfully by host ${hostId}`);
...
export const joinRoom = async (req, res) => {
  try {
    ...
    console.log(`[Join Room API] Opponent ${opponentId} attempting to join room ${normalizedRoomId}`);

    // Check if the opponent user actually exists
    const opponentDetails = await User.findById(opponentId);
    if (!opponentDetails) {
      console.log(`❌ [Join Room API] Opponent ${opponentId} not found in database`);
      return res.status(404).json({ error: "Opponent user not found" });
    }

    // First, check if room exists at all
    const existingRoomCheck = await Room.findOne({ roomId: normalizedRoomId });
    if (!existingRoomCheck) {
      console.log(`❌ [Join Room API] Room ${normalizedRoomId} does not exist in database`);
      return res.status(404).json({ error: "Room not found. Please check the room ID." });
    }
    
    console.log(`[Join Room API] Room ${normalizedRoomId} exists. Status: ${existingRoomCheck.status}, Has opponent: ${!!existingRoomCheck.opponent}`);
    ...
    if (!updatedRoom) {
      ...
      // Let's check what the actual issue is for better error messages
      const existingRoom = await Room.findOne({ roomId: normalizedRoomId });
      
      if (!existingRoom) {
        console.log(`❌ Room ${normalizedRoomId} does not exist`);
        return res.status(404).json({ error: "Room not found. Please check the room ID." });
      }
      
      if (existingRoom.status !== 'waiting') {
        console.log(`❌ Room ${normalizedRoomId} is not in waiting status (current: ${existingRoom.status})`);
        return res.status(400).json({ error: `Room is not available (status: ${existingRoom.status})` });
      }
      
      if (existingRoom.opponent) {
        console.log(`❌ Room ${normalizedRoomId} already has an opponent`);
        return res.status(400).json({ error: "Room is already full" });
      }
      
      if (existingRoom.host.toString() === opponentId) {
        console.log(`❌ User ${opponentId} is trying to join their own room ${normalizedRoomId}`);
        return res.status(400).json({ error: "You cannot join your own room" });
      }
      
      // Fallback error
      console.log(`❌ Unknown error joining room ${normalizedRoomId}`);
      return res.status(404).json({ error: "Unable to join room. Please try again." });
    }
...
export const cancelRoom = async (req, res) => {
  ...
  try {
    ...
    const normalizedRoomId = roomId.trim().toUpperCase();
    console.log(`[Cancel Room API] User ${userId} attempting to cancel room ${normalizedRoomId}`);

    // First, find the room to check if user is authorized
    const room = await Room.findOne({ roomId: normalizedRoomId });
    
    if (!room) {
      console.log(`❌ [Cancel Room API] Room ${normalizedRoomId} not found`);
      return res.status(404).json({ error: "Room not found" });
    }

    // Check if user is host or opponent (convert ObjectIds to strings for comparison)
    const isHost = room.host.toString() === userId;
    const isOpponent = room.opponent?.toString() === userId;
    
    if (!isHost && !isOpponent) {
      console.log(`❌ [Cancel Room API] User ${userId} is not part of room ${normalizedRoomId}`);
      return res.status(403).json({ error: "You are not authorized to cancel this room" });
    }

    // Delete the room
    await Room.findOneAndDelete({ roomId: normalizedRoomId });
    console.log(`✅ [Cancel Room API] Room ${normalizedRoomId} cancelled by ${isHost ? 'host' : 'opponent'} ${userId}`);
...
export const startMatch = async (req, res) => {
  try {
    ...
    room.createdAt = new Date(startTimeMs); // Reset to actual start moment
    await room.save();

    // --- Emit Socket Event ---
    const io = getIO();
    // Emit to everyone in the room with startTime as unix ms number
    io.to(normalizedRoomId).emit("match-started", { 
      roomId: normalizedRoomId,
      startTime: startTimeMs  // Always a number, never a Date string
    });

    console.log(`✅ Match started in room ${normalizedRoomId}`);
    res.status(200).json({ message: "Match started", startTime: startTimeMs });
  } catch (err) {
